                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center justify-end gap-2">
                        ...
                        <form action="{{ route('anak-asuh.toggle-status', $anak) }}" method="POST">
                            @csrf @method('PATCH')
                            <button type="submit" class="text-xs px-3 py-1.5 rounded-lg border font-medium transition-colors {{ $anak->status_anak === 'Aktif' ? 'border-slate-300 text-slate-600 hover:bg-slate-100' : 'border-green-300 text-green-700 hover:bg-green-50' }}"
                                    onclick="return confirm('Ubah status {{ $anak->nama_anak }}?')">
