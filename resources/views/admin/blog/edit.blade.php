...
@section('page-subtitle', 'Perbarui konten artikel atau berita yayasan')
...
<div class="max-w-3xl">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm">
        <div class="border-b border-slate-100 px-6 py-4">
            <h2 class="font-semibold text-slate-700">Form Edit Artikel</h2>
        </div>

        <form action="{{ route('admin.blog.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p id="slug-preview" class="text-xs text-slate-400 mt-1">URL: /blog/{{ $article->slug }}</p>
                ...
                <div>
                    ...
                    <input type="date" name="published_date" id="published_date" value="{{ old('published_date', $article->created_at->format('Y-m-d')) }}"
                    ...
                    <p class="text-xs text-slate-400 mt-1">Ubah jika ingin menyesuaikan tanggal riwayat artikel.</p>
            ...
            <div>
                ...
                @if($article->image)
                    <div class="mb-3">
                        ...
                        <img src="{{ asset('storage/' . $article->image) }}" alt="Current Image" class="h-32 object-cover rounded-xl border border-slate-200">
                    </div>
                @endif
                ...
                <div id="img-preview" class="hidden mt-3">
                    ...
                    <img id="img-preview-src" src="#" alt="Preview" class="h-32 object-cover rounded-xl border border-slate-200">
                </div>
                <p class="text-xs text-slate-400 mt-1">Format: JPG, PNG, WebP. Maks 3 MB. Kosongkan jika tidak ingin mengubah gambar.</p>
...
function generateSlugPreview(title) {
    ...
    preview.textContent = slug ? 'URL preview: /blog/' + slug + '-xxxxx' : '';
}
