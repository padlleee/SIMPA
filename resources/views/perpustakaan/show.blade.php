...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 space-y-4">
        ...
        <div class="flex gap-2">
            ...
            <form action="{{ route('perpustakaan.destroy', $perpustakaan) }}" method="POST" class="flex-1">
                @csrf @method('DELETE')
                <button type="button" onclick="confirmDelete(this.form)"
                        class="w-full flex items-center justify-center gap-2 border border-red-200 text-red-500 py-2.5 rounded-xl text-sm font-medium hover:bg-red-50 transition-colors">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div class="flex items-start justify-between gap-4 mb-4">
                ...
                <span class="text-xs font-semibold px-2.5 py-1 rounded-full
                    {{ in_array($perpustakaan->kondisi_buku, ['Baik','Cukup Baik']) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600' }}">
                    {{ $perpustakaan->kondisi_buku ?? 'Baik' }}
                </span>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mt-0.5">Data buku dan riwayat peminjaman akan dihapus permanen.</p>
            </div>
        </div>
        <div class="flex gap-3">
            <button onclick="document.getElementById('deleteModal').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-700 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-colors">Batal</button>
            <button id="deleteConfirmBtn" class="flex-1 bg-red-600 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-red-700 transition-colors">Ya, Hapus</button>
        </div>
    </div>
</div>
<script>
...
document.getElementById('deleteConfirmBtn').addEventListener('click', () => { if(_deleteForm) _deleteForm.submit(); });
